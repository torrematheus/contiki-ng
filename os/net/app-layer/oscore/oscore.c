...
size_t oscoap_prepare_external_aad(coap_message_t* coap_pkt, opt_cose_encrypt_t* cose, uint8_t* buffer, uint8_t sending){
  ...
  ret += cbor_put_unsigned(&buffer, (coap_pkt->context->alg));
  ...
  if(sending == 1){
    if(coap_is_request(coap_pkt)) {
      ...
      ret += cbor_put_bytes(&buffer, coap_pkt->context->sender_context->sender_id_len, coap_pkt->context->sender_context->sender_id);
      ...
      ret += cbor_put_bytes(&buffer, coap_pkt->context->recipient_context->recipient_id_len, coap_pkt->context->recipient_context->recipient_id);
      ret += cbor_put_bytes(&buffer, seq_len, seq_buffer);
    } 
  } else {
